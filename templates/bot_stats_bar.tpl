...
{if $gBitSystem->isFeatureActive( 'messages_site_contact' )}
	<a href="{$smarty.const.MESSAGES_PKG_URL}contact.php">{biticon ipackage="icons" iname="internet-mail" ilocation=menu iexplain="Contact Us"}</a> 
{/if}
	<a id="poweredby" class="external pull-right" href="https://www.bitweaver.org"><img src="/liberty/icons/bitweaver/bitweaver.gif" alt="Bitweaver" title="Bitweaver" class="icon"></a> {$version_info.local}
	{if $gBitUser->isAdmin()} <strong>{$gBitSystem->getBitVersion()}</strong>
		...
		{elseif $version_info.compare eq 0 && !$version_info.release}
