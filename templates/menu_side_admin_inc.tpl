{strip}
<a tabindex="-1" accesskey="A" class="{if $gBitSystem->getActivePackage() eq 'kernel'} active{/if}" href="{if $gBitUser->isAdmin()}{$smarty.const.KERNEL_PKG_URL}admin/index.php{else}#{/if}">
	<i class="icon-cog"></i> {tr}Administration{/tr}
</a>
<ul class="dropdown-menu sub-menu pull-right">	
	{foreach key=key item=menu from=$adminMenu}
		{if $key eq 'kernel' || $key eq 'liberty' || $key eq 'users' || $key eq 'themes'}
	...
	{/foreach}
	{foreach key=key item=menu from=$adminMenu}
		{if $key neq 'kernel' && $key neq 'liberty' && $key neq 'users' && $key neq 'themes'}
